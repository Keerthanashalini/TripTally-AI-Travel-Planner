...
TripTally is an AI-powered travel planning application that helps users create fully customized and intelligent travel itineraries in just seconds. Users can define their preferences—such as departure location, destination, duration, and budget—and TripTally will instantly generate a complete day-by-day plan including activities, transportation, hotel suggestions, and local travel tips.

Built using Gemini AI, Firebase, and the modern React ecosystem, TripTally simplifies the trip planning experience and turns it into an interactive, efficient, and enjoyable process.

## 1. Project Overview

The goal of TripTally is to eliminate the hassle and complexity of manually planning trips. Whether you're a solo traveler or a family on vacation, finding activities, coordinating hotel stays, and organizing schedules can be overwhelming and time-consuming.

TripTally addresses this by offering an intelligent, AI-assisted planning tool that generates personalized itineraries in real-time. The app captures user inputs through a guided step-by-step interface, processes them using Gemini AI, and presents structured travel recommendations instantly—empowering users with both flexibility and automation.
...
```
├── src/
│   ├── api/               # API connections: Firebase, Gemini, Geoapify
│   ├── components/        # Reusable UI components (Button, Dialog, Badge, etc.)
│   ├── hooks/             # Custom React hooks (theme, scrollToTop)
│   ├── pages/             # Main pages (Home, CreateTrip, SavedTrip, DetailTrip)
│   ├── store/             # Zustand store (useTripStore, useAuthStore)
│   ├── config/            # State, constants, emojiMap, prompt templates
│   ├── App.tsx            # Root routing with React Router
